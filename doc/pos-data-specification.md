...
The order request is generated by the customer app to communicate the products
in the cart of the user. This includes the following data:

### Products

The list of products added to cart by the user. *(array with sentinel)*
...
- **Bits 4 and 5** specify the length of the ordered stock of the item.
The length is retrieved by the value in the bits, plus one.

- **Bits 6 and 7** are reserved, explicitly set to 0.
...
The UUID is 16-bytes large. To reduce the size, the specification only mentions
enough bytes from the end which can uniquely identify the UUID.

#### Ordered Stock

The ordered stock of the item, which the user has added to their cart. Zero
should not be reported as then the entry would simply be wastage of space.
*(1 to 4 bytes, little-endian, variable)*
